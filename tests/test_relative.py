...
class TestRelativeDate(object):
    ...
    def test_adding_two_relative_dates_adds_offsets(self):
        ...
        assert off_by_one + off_by_two == RelativeDate(
            timedelta(days=3), clock=FakeDate.today
        )
    ...
    def test_proxy_formatting_call_to_underlying_date(self):
        ...
        assert pattern.format(RelativeDate(clock=FakeDate.today)) == pattern.format(
            TODAY
        )
...
class TestRelativeDeltaInterop(object):
    ...
    def test_add_relative_delta_instances(self):
        ...
        assert rd + rd == RelativeDate.fromdate(
            date(2016, 1, 1), offset=relativedelta(days=2)
        )
